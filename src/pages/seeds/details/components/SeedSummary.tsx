...
import { loadDetailsShowNonPresentFromLocalStorage, saveDetailsShowNonPresentToLocalStorage } from '../../../../api/services/seed-browser/SeedService';
...
class SeedSummary extends React.Component<Props, State> {
    ...
    reportSeedAsInvalid = () => {
        Modal.confirm({
          title: 'Invalid seed',
          content: 'This seed will be marked for manual verification. Before submitting please make sure you are not using any world generation altering mods.',
          okText: 'Report',
          cancelText: 'Cancel',
    ...
    render() {
        const { seed, gameUpgrades, geyserTypes, spaceDestinationTypes, onReportInvalid } = this.props;
            ...
            <Card className="shadow-card card-full-width" bordered={false} >
                <div style={{ display: 'flex', flexFlow: 'column wrap', justifyContent: 'center', alignContent: 'center', width: '100%' }} >
                    <Row type="flex" style={{ justifyContent: 'flex-end', marginTop: -8 }}>
                        <a><h4 className="hover-text-danger" onClick={this.reportSeedAsInvalid}>Report as invalid</h4></a>
                    </Row>
                    ...
                    <Row type="flex" style={{ justifyContent: 'center', marginBottom: 30 }}>
                        {
                            Object.keys(spaceDestinationTypes).map((spaceDestinationType: string, index: number) => {
                                ...
                                return (<GeyserChip key={index} count={count} label={spaceDestinationTypes[spaceDestinationType].displayName} imgName={imageFile} />)
                            })
                        }
                    </Row>
                    ...
                    <Row type="flex" style={{ justifyContent: 'space-between', alignItems: 'flex-end' }}>
                        ...
                        <div style={{ display: 'inline-flex', alignItems: 'flex-end' }}>
                            <h4 style={{ margin: '0px 5px 0px 0px' }}>Show geyser and planet types not present on the map</h4>
